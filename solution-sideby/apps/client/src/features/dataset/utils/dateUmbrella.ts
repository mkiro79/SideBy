...
export function createDateUmbrella(
  ...
  let allKeys = new Set([...Object.keys(groupAByKey), ...Object.keys(groupBByKey)]);
 ...
 */
function formatUmbrellaLabel(key: string, granularity: DateGranularity): string {
 ...
 */
function generateKeysInRange(minKey: string, maxKey: string, granularity: DateGranularity): string[] {
  const keys: string[] = [];
